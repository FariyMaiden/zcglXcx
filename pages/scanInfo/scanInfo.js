...
Page({
  ...
  requestEquiment:function(num){
    ...
    wx.request({
      url: 'https://www.trjiot.cn/weixin.asmx/GetAssetInfo',
      data: { 
        assetNo: num ,
        evalue: evalue
      },
  ...
  downloadImage:function(num){
    var that = this
     wx.downloadFile({
       url: 'https://www.trjiot.cn/Images/'+num+'.jpg',
...
//  显示或隐藏地址选项
  ...
  sureUpload:function(){
    ...
    // 上传设备信息
   wx.request({
     url: 'https://www.trjiot.cn/weixin.asmx/UpdateAssetCheckInfo',
     data: param,
     header: { 'content-type': 'application/x-www-form-urlencoded'},
     method: 'POST',
     success: function(res) {
   ...
   var evalue = app.Encrypt()
    ...
    if(that.data.is_takePhoto){
      ...
      wx.uploadFile({
        url: 'https://www.trjiot.cn/weixin.asmx/SaveImage',
